...
function update_address(hash, txid, amount, type, cb) {
  ...
  find_address(hash, function(address) {
    var masternode_ip = '';
    lib.get_masternode_ip_by_address(hash , function(nodeip) {
      masternode_ip = nodeip;
      if (address) {
...
function update_address_ripple(hash, ledgerHash, txid, amount, type, cb) {
  ...
  lib.get_account_balance(hash, ledgerHash, function(balance) {
    find_address(hash, function (address) {
      var masternode_ip = '';
      lib.get_masternode_ip_by_address(hash , function( masternode_ip ){});
      if (address) {
        // ensure tx doesnt already exist in address.txs
        lib.is_unique(address.txs, txid, function (unique, index) {
          var tx_array = address.txs;
              ...
              lib.syncLoop(vin.length, function (loop) {
                ...
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  lib.calculate_total(vout, function(total){
                    ...
                    var totalAmt = 0;
                    if(nvin.length > 0 || 'undefined' === typeof(tx.Input) || 'undefined' === typeof(tx.Output) || tx.Input > tx.Output)
                    {
                      totalAmt = total;
                    }
                    else
                    {
                      totalAmt = (tx.Output - tx.Input) * 100000000;
                      var poolindex = vout.length-2;
                      vout[poolindex].amount = totalAmt - vout[vout.length-1].amount;
                      console.log('poolindex=' + poolindex + " poolvalue=" + vout[poolindex].amount + " vout.length=" + vout.length);
...
module.exports = {
  ...
  check_masternode_addresses(masternodehash, cb) {
    Address.find({a_id:masternodehash, masternode_ip:{$ne: ""}/*, {a_id:1, masternode_ip:1,balance:1,received:1}*/}, {a_id:1, masternode_ip:1,balance:1,received:1}, function(err, address) {
      if(address.length > 0) {
        console.log("master");
        return cb(1);
      } else {
  ...
  get_last_real_txs: function(count, min, cb) {
    Tx.find({'total': {$gt: min}, 'vin':{$elemMatch:{$ne:null}}, 'vin': {$elemMatch: {addresses: {$ne:'coinbase'}}}}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_last_pool_by_time: function(second, cb) {
    Tx.find({'timestamp': {$gt: second}, $or:[{'vin': {$eq:[]}},{'vin': {$elemMatch: {addresses: 'coinbase'}}}]}).sort({_id: 'desc'}).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_vpub_txs: function(second, amount, address, cb) {
    if(address == '')
    {
      Tx.find({'timestamp': {$gt: second}, 'total': {$gte: amount}, 'vin':{$elemMatch:{$ne:null}}, 'vin': {$elemMatch: {addresses: {$ne:'coinbase'}}}}).sort({_id: 'desc'}).exec(function(err, txs){
        if (err) {
          return cb(err);
        } else {
          return cb(txs);
    ...
    {
      Tx.find({'timestamp': {$gt: second}, 'total': {$gte: amount}, 'vin': {$elemMatch:{'addresses' : address}}}).sort({_id: 'desc'}).exec(function(err, txs){
        if (err) {
          return cb(err);
        } else {
          return cb(txs);
  ...
  get_masternode_rewards: function(since, mnPayees, cb) {
    Tx.aggregate([
      { $match: {
          "timestamp": { $gt: since },
          $or:[{'vin': {$eq:[]}},{'vin': {$elemMatch: {addresses: 'coinbase'}}}]
      }},
      { "$unwind": "$vout"  },
      { $match: {
  ...
  get_masternode_rewards_by_address: function(mnPayees, cb) {
    Tx.aggregate([
      { $match: {
        "timestamp": { $gt: 0 },
        $or:[{'vin': {$eq:[]}},{'vin': {$elemMatch: {addresses: 'coinbase'}}}]
  ...
  get_masternode_rewards_detail_by_address: function(mnPayees, mnTransactiontime, mnTransactionEndtime, cb) {
    Tx.aggregate([
      { $match: {
...
/*
  ...
  get_masternode_addresses: function(cb) {
    Address.find({masternode_ip : {$ne: ""}/*, {a_id:1, masternode_ip:1,balance:1,received:1}*/}, {a_id:1, masternode_ip:1,balance:1,received:1}, function(err, address) {
      if(address) {
        return cb(address);
      } else {
        return cb();
