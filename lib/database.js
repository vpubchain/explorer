...
module.exports = {
  ...
  get_last_txs_by_time: function(second, cb) {
    Tx.find({'timestamp': {$gt: second}}).sort({_id: 'desc'}).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_txs: function(second, amount, address, cb) {
    if(address == '')
    {
      console.log('test1');
      Tx.find({'timestamp': {$gt: second}, 'total': {$gte: amount}}).sort({_id: 'desc'}).exec(function(err, txs){
        if (err) {
          return cb(err);
        } else {
          return cb(txs);
...
/*
  get_txs_num: function(second, amount, cb) {
    console.log('test1');
    Tx.find({'timestamp': {$gt: second}, 'total': {$gte: amount}}).sort({_id: 'desc'}).exec(function(err, txs){
      if (err) {
        return cb(err);
...
/*
    Tx.find({'timestamp': {$gte: begin}}, {blockhash:1}, function(err, txs){
      if (err) {
        console.log(err);
        return cb(err);
      } else {
