              ...
              lib.syncLoop(vin.length, function (loop) {
                ...
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  lib.calculate_total(vout, function(total){
                    ...
                    if(nvin[0].addresses == "coinbase") {
                      lib.syncLoop(vout.length, function(voutLoop){
                        var xxx = voutLoop.iteration();
                        update_address_rewards(vout[xxx].addresses, vout[xxx].amount, function(){                         
                          voutLoop.next();
                        });
                    ...
                    var performancefundreward = 0;
                    var ecologicalfundreward = 0;
                    var developmentfundreward = 0;
                    var destroyfundreward = 0;
                    var pensionfundreward = 0;

                    vout.forEach(ot => {
                      switch (ot.addresses) {
                        case settings.ecological_funds_address:
                          ecologicalfundreward = ot.amount;
                          break;
                        case settings.performance_funds_address:
                          performancefundreward = ot.amount;
                          break;
                        case settings.development_team_fund_address:
                          developmentfundreward = ot.amount;
                          break;
                        case settings.destroy_fund_address:
                          destroyfundreward = ot.amount;
                          break;
                        case settings.pension_fund_address:
                          pensionfundreward = ot.amount;   
                        default:
                          break;
                      }
                    });
                    ...
                    var newTx = new Tx({
                      ...
                      iscoinbase: nvin[0].addresses == "coinbase" ? true : false,
                      performancefundreward: performancefundreward,
                      ecologicalfundreward: ecologicalfundreward,
                      developmentfundreward: developmentfundreward,
                      destroyfundreward: destroyfundreward,
                      pensionfundreward: pensionfundreward
                    });
...
module.exports = {
  ...
  get_last_real_txs: function(count, min, cb) {
    Tx.find({iscoinbase: false, 'total': {$gt: min}}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_real_txs: function(min, cb) {
    Tx.find({iscoinbase: false, 'total': {$gt: min}}, {_id:0, "total":1, "timestamp":1}).sort({_id: 'desc'}).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_last_pool_by_time: function(second, cb) {
    Tx.find({'timestamp': {$gt: second}, iscoinbase: true}).sort({_id: 'desc'}).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
  ...
  get_bigcharge_txs: function(second, amount, address, cb) {
    if(address == '')
    {
      Tx.find({iscoinbase: false, 'timestamp': {$gt: second}, 'total': {$gte: amount}}, {_id:0, "total":1, "timestamp":1, "txid":1 , "vin": 1}).sort({_id: 'desc'}).exec(function(err, txs){
        if (err) {
          return cb(err);
        } else {
          return cb(txs);
    ...
    {
      Tx.find({iscoinbase: false, 'timestamp': {$gt: second}, 'total': {$gte: amount}}).sort({_id: 'desc'}).exec(function(err, txs){
        if (err) {
          return cb(err);
        } else {
          return cb(txs);
...
/*
   ...
   * @param {*} cb 
   */
  get_performance_supply: function(starttime, endtime, cb) {
    var start = +(starttime);
    var end = +(endtime);
    console.log("start=", start, "end = ", end);
    Tx.aggregate([
      ...
      {  $group: { _id: null,  performancefundreward: { $sum: "$performancefundreward" } } },
    ], function(err, data) {
      if (err) {
        console.log(err);
        return cb(null);
        ...
        return cb(data[0].performancefundreward);
      }
    });
  },
