    ...
    if (settings.nethash == 'netmhashps') {
      ...
      request({uri: uri, json: true}, function (error, response, body) { //returned in mhash
        if (body.netmhashps) {
          if (settings.nethash_units == 'K') {
            ...
            return cb((body.netmhashps * 1000000).toFixed(2));
          } else if (settings.nethash_units == 'T') {
            return cb((body.netmhashps / 1000000).toFixed(2));
          } else if (settings.nethash_units == 'P') {
            return cb((body.netmhashps / 1000000000).toFixed(2));
          } else {
            return cb(body.netmhashps.toFixed(2));
          }
        } else {
          return cb('-');
        }
      ...
      request({uri: uri, json: true}, function (error, response, body) {
        if (body == 'There was an error. Check your console.') {
          ...
          if (settings.nethash_units == 'K') {
            return cb((body / 1000).toFixed(2));
          } else if (settings.nethash_units == 'M'){
            return cb((body / 1000000).toFixed(2));
          } else if (settings.nethash_units == 'G') {
            return cb((body / 1000000000).toFixed(2));
          } else if (settings.nethash_units == 'T') {
            return cb((body / 1000000000000).toFixed(2));
          } else if (settings.nethash_units == 'P') {
            return cb((body / 1000000000000000).toFixed(2));
          } else {
            return cb((body).toFixed(2));
          }
        }
      });
    }
