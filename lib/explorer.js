...
module.exports = {
  ...
  get_masternodelist: function(cb) {
    ...
    request({uri: uri, json: true}, function (error, response, body) {
      ...
      {    
        ...
        console.log("get_masternode_ip_by_address. ", ' body= ', body, ' key', key , 'masternode_data', masternode_data);
  ...
  get_masternode_status_by_address: function(address, cb)
  {
    if (settings.baseType === 'pivx')
      ...
      if (vout[i].type != 'nonstandard' && vout[i].type != 'nulldata' && 'undefined' != typeof(vout[i].value) && 'undefined' != typeof(vout[i].scriptPubKey) && vout[i].scriptPubKey.hasOwnProperty("addresses")) { 
        ...
        module.exports.is_unique(arr_vout, vout[i].scriptPubKey.addresses[0], function(unique, index) {
            ...
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              var isCoinStake = false;
              if(vout[i].scriptPubKey.asm.indexOf("OP_ISCOINSTAKE") >= 0)
              {
                isCoinStake = true;
              }
