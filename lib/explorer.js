...
var bitnodes_url = 'http://127.0.0.1:5000/vpbitnodes/api/v1.0/';

email = {
  service: 'QQ',
  user: settings.email_sender,
  pass: settings.email_passwd,
}
...
module.exports = {
    ...
    if (settings.nethash == 'netmhashps') {
      ...
      request({uri: uri, json: true}, function (error, response, body) { //returned in mhash
        if (body.netmhashps) {
          if (settings.nethash_units == 'K') {
            return cb((body.netmhashps * 1000).toFixed(6));
          } else if (settings.nethash_units == 'G') {
            return cb((body.netmhashps / 1000).toFixed(6));
          } else if (settings.nethash_units == 'H') {
            return cb((body.netmhashps * 1000000).toFixed(6));
          } else if (settings.nethash_units == 'T') {
            return cb((body.netmhashps / 1000000).toFixed(6));
          } else if (settings.nethash_units == 'P') {
            return cb((body.netmhashps / 1000000000).toFixed(6));
          } else {
            return cb(body.netmhashps.toFixed(6));
          }
        } else {
          return cb('-');
        }
      ...
      request({uri: uri, json: true}, function (error, response, body) {
        if (body == 'There was an error. Check your console.') {
          ...
          if (settings.nethash_units == 'K') {
            return cb((body / 1000).toFixed(6));
          } else if (settings.nethash_units == 'M'){
            return cb((body / 1000000).toFixed(6));
          } else if (settings.nethash_units == 'G') {
            return cb((body / 1000000000).toFixed(6));
          } else if (settings.nethash_units == 'T') {
            return cb((body / 1000000000000).toFixed(6));
          } else if (settings.nethash_units == 'P') {
            return cb((body / 1000000000000000).toFixed(6));
          } else {
            return cb((body).toFixed(6));
          }
        }
      });
    }
  ...
  addnode: function(address, cb) {
    var paramsData = [];
    paramsData.push(address);
    paramsData.push('onetry');
    request(request({
      method: 'POST',
      ...
      if (body) {
        console.log("addnode return body", body);
        return cb(true);
      } else {
  ...
  get_location: function(x, y, cb){
    var uri = 'http://api.map.baidu.com/geocoder/v2/?ak=efaf6IBwbYlyR9UGQlugSAqIIOWLcDoK&location=' + x + ',' + y + '&output=json&pois=1';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },

  // get_bitnodes_url: function(header, cb) {
  //   var uri = bitnodes_url + header;
    
  //   console.log('get_bitnodes_url' + uri);
  //   request({uri: uri, json: true}, function (error, response, body) {
  //     return cb(body);
  //   });
  // },
  ...
  get_bitnodes_dashborad_url: function(header, days, cb) {
    var uri = bitnodes_url + header;
    if(days != 1)
    {
      uri = uri + '?days=' + days;
    }
    console.log('get_bitnodes_dashborad_url' + uri);
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },

  get_bitnodes_vpubborad_url: function(header, days, cb) {
    var uri = bitnodes_url + header + '/' + days;
    console.log('get_bitnodes_vpubborad_url' + uri);
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
  ...
  get_coldstakinginfo: function(address, cb) {
    var uri = 'http://' + address + '/' + settings.port + 'getcoldstakinginfo';
    request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
  ...
  get_masternode_ip_by_address: function(address, cb)
  {
    if (settings.baseType === 'pivx')
      var uri = base_url + 'masternode?command=list' + '&filter=' + address;
    else
      var uri = base_url + 'masternodelist?mode=full' + '&filter=' + address;
    request({uri: uri, json: true}, function (error, response, body) {
      var masternode_ip = '';
      if(body != 'There was an error. Check your console.')
      {
        var masternode_list = body;
        
        for (var key in masternode_list)
        {
          
          var masternode_data = masternode_list[key];
          console.log("get_masternode_ip_by_address. address=:", address, ' body= ', body, ' key', key , 'masternode_data', masternode_data);
          masternode_ip = masternode_data.ip;
          /*var mnData = masternode_data.split(/(\s+)/).filter( function(e) { return e.trim().length > 0; } );
          if (settings.masternodes.list_format.ip === 0)
            masternode_ip = key.trim().replace(':'+settings.masternodes.default_port, '');
          else if (settings.masternodes.list_format.ip > -1)
            masternode_ip = mnData[settings.masternodes.list_format.ip - 1].trim().replace(':'+settings.masternodes.default_port, '');
          */
          }
      }
      console.log("get_masternode_ip_by_address. address=:", address, ' body= ', body, ' masternodeip= ', masternode_ip);
      return cb(masternode_ip);
    });
  },
  ...
  get_rawtransaction: function(hash, cb) {
    // var uri = base_url + 'getrawtransaction?txid=' + hash;
    ...
    request({uri: uri, json: true}, function (error, response, body) {
      ...
      if(tx.vout && tx.vout.length) {
        ...
        module.exports.get_rawtransaction(hash, cb)
      }
    });
  },
  ...
  prepare_vout: function(vout, txid, vin, cb) {
    ...
    module.exports.syncLoop(vout.length, function (loop) {
      ...
      if (vout[i].type != 'nonstandard' && vout[i].type != 'nulldata' && 'undefined' != typeof(vout[i].value) && 'undefined' != typeof(vout[i].scriptPubKey) && vout[i].scriptPubKey.hasOwnProperty("addresses")) { 
        ...
        module.exports.is_unique(arr_vout, vout[i].scriptPubKey.addresses[0], function(unique, index) {
            ...
            module.exports.convert_to_satoshi(parseFloat(vout[i].value), function(amount_sat){
              var isCoinStake = vin[0].addresses == "coinbase" ? true :false;
              if(unique){
                arr_vout.push({addresses: vout[i].scriptPubKey.addresses[0], amount: amount_sat, iscoinstake: isCoinStake});
              }else {
                arr_vout[index] = {addresses: vout[i].scriptPubKey.addresses[0], amount: amount_sat, iscoinstake: isCoinStake}
              }
              //console.log("addresses: ", vout[i].scriptPubKey.addresses[0]);
              amount_sum = amount_sum + amount_sat;
              loop.next();
            });
        });
      } else {
        ...
        if (tx) {
          module.exports.syncLoop(tx.vout.length, function (loop) {
            ...
            if (tx.vout[i].n == input.vout) {
              ...
              if (tx.vout[i] && tx.vout[i].scriptPubKey.addresses) {
