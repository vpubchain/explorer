...
function get_summary(coin, exchange, Crymktid, cb) {
  ...
  request({uri: base_url + '/' + Crymktid + '/ticker', json: true}, function (error, response, body) {
    if (error) {
      ...
      summary['bid'] = body.data['bid'].toFixed(6);
      summary['ask'] = body.data['ask'].toFixed(6);
      ...
      var sells = [];
            if (orders['buyorders'].length > 0){
                for (var i = 0; i < orders['buyorders'].length; i++) {
                    var order = {
                        amount: parseFloat(orders.buyorders[i].quantity).toFixed(6),
                        price: parseFloat(orders.buyorders[i].price).toFixed(6),
                        //  total: parseFloat(orders.buyorders[i].total).toFixed(6)
                        // Necessary because API will return 0.00 for small volume transactions
                        total: (parseFloat(orders.buyorders[i].quantity).toFixed(6) * parseFloat(orders.buyorders[i].price)).toFixed(6)
                    }
                    buys.push(order);
                }
                } else {}
                ...
                for (var x = 0; x < orders['sellorders'].length; x++) {
                    var order = {
                        amount: parseFloat(orders.sellorders[x].quantity).toFixed(6),
                        price: parseFloat(orders.sellorders[x].price).toFixed(6),
                        //    total: parseFloat(orders.sellorders[x].total).toFixed(6)
                        // Necessary because API will return 0.00 for small volume transactions
                        total: (parseFloat(orders.sellorders[x].quantity).toFixed(6) * parseFloat(orders.sellorders[x].price)).toFixed(6)
                    }
                    sells.push(order);
                }
            } else {
